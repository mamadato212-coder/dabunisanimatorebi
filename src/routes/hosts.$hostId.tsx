...
export const Route = createFileRoute('/hosts/$hostId')({
  ...
  notFoundComponent: () => (
    <div className="max-w-xl mx-auto p-10 text-center">
      ...
      <Link to="/hosts" className="mt-6 inline-block px-5 py-2.5 rounded-full bg-accent text-white font-bold text-sm">Back</Link>
    </div>
  ),
});
...
function HostDetail() {
  ...
  const a = (t.hosts || []).find((x: any) => x.id === hostId);
  if (!a) throw notFound();
  ...
  return (
    <div className="max-w-[1300px] mx-auto px-4 md:px-10 py-8 md:py-14">
      <Link to="/hosts" className="inline-flex items-center gap-1 text-xs font-bold uppercase tracking-widest text-muted-foreground hover:text-accent mb-6">
        <ChevronLeft size={14} /> {lang === 'ka' ? 'უკან ანიმATOREBze' : 'Back to animators'}
      </Link>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
        <div className="flex flex-col gap-3">
          ...
          {gallery.length > 1 && (
            <div className="flex gap-2 overflow-x-auto">
              {gallery.map((img, i) => (
                <button key={i} onClick={() => setActive(i)} className={`w-20 h-20 rounded-2xl overflow-hidden border-2 flex-shrink-0 ${active === i ? 'border-accent' : 'border-transparent'}`}>
        ...
        <div className="flex flex-col gap-5">
          <div className="flex items-center gap-2 flex-wrap">
            <span className="text-[10px] font-bold uppercase tracking-[0.3em] text-accent">{a.category}</span>
          </div>
            ...
            <ul className="flex flex-col gap-2 mt-2">
              {features.map((f, i) => (
                <li key={i} className="flex items-start gap-2 text-sm font-semibold">
                  <span className="w-5 h-5 rounded-full bg-accent/15 text-accent flex items-center justify-center flex-shrink-0 mt-0.5"><Check size={12} /></span>
                  {f}
                </li>
              ))}
            </ul>
          ...
          <Link to="/booking" className="mt-2 inline-flex items-center justify-center gap-2 px-8 py-4 rounded-full bg-accent text-white font-bold uppercase tracking-widest hover:scale-105 transition-all shadow-glow w-fit">
          ...
          <div className="flex flex-col gap-3">
            {faq.map((q: any, i: number) => (
              <details key={i} className="rounded-2xl bg-card border border-border p-5 group">
                <summary className="font-bold cursor-pointer flex justify-between items-center">{q.question}<span className="text-accent group-open:rotate-180 transition-transform">▾</span></summary>
